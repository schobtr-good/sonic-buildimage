...
static int cls_fpga_probe(struct platform_device *pdev)
{
	...
	if (res) {
		/* use devm_ioremap_resource to map whole fpga res again will be
		 * conflict
		 */
	...
	pr_info("board type val: 0x%08x\n", fpga_type);
	if (fpga_type != 0x01) {
		pr_info("unknow board type val: 0x%08x\n", fpga_type);
		goto mem_unmap;
	}

	pr_info("FPGA version: 0x%x\n", ioread32(fpga->base + FPGA_VERSION));

	ret = sysfs_create_group(&pdev->dev.kobj, &fpga_attr_grp);
	if (ret != 0) {
		pr_err("Cannot create FPGA system sysfs attributes\n");
		goto err_remove_fpga;
	}
	// Set default read address to VERSION
	fpga->fpga_read_addr = fpga->base + FPGA_VERSION;
	return 0;
...
MODULE_AUTHOR("CLS");
MODULE_DESCRIPTION("Celestica MRVL_CPO1 Base FPGA system driver");
