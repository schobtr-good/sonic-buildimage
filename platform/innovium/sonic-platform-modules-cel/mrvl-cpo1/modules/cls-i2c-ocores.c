...
static void ocores_process(struct ocores_i2c *i2c, u8 stat) {
    ...
    if (i2c->state == STATE_READ) {
        oc_setreg(i2c, OCI2C_CMD,
                  i2c->pos == (msg->len - 1) ? OCI2C_CMD_READ_NACK
                                             : OCI2C_CMD_READ_ACK);
    } else {
...
static irqreturn_t ocores_isr(int irq, void *dev_id) {
    ...
    if (!(stat & OCI2C_STAT_IF)) return IRQ_NONE;
 ...
 */
static int ocores_wait(struct ocores_i2c *i2c, int reg, u8 mask, u8 val,
...
static void ocores_process_polling(struct ocores_i2c *i2c) {
    while (1) {
        ...
        if (ret == IRQ_NONE) break; /* all messages have been transferred */
    }
}

static int ocores_xfer_core(struct ocores_i2c *i2c, struct i2c_msg *msgs,
...
static int ocores_xfer_polling(struct i2c_adapter *adap, struct i2c_msg *msgs,
                               int num) {
...
static int ocores_i2c_of_probe(struct platform_device *pdev,
    ...
    if (i2c->ip_clock_khz == 0) {
        if (of_property_read_u32(np, "opencores,ip-clock-frequency", &val)) {
            ...
            dev_warn(&pdev->dev,
                     "Deprecated usage of the 'clock-frequency' property, "
                     "please update to 'opencores,ip-clock-frequency'\n");
        } else {
    ...
    of_property_read_u32(pdev->dev.of_node, "reg-io-width", &i2c->reg_io_width);
...
static int ocores_i2c_remove(struct platform_device *pdev) {
    ...
    if (!IS_ERR(i2c->clk)) clk_disable_unprepare(i2c->clk);
