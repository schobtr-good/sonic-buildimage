...
#define DRIVER_NAME "cls-i2c-xiic"
...
#define GL_ISR_OFFSET 0x00 /* CLS MRVL_CPO1 FPGA global interrupt status reg*/
#define GL_IER_OFFSET 0x08 /* CLS MRVL_CPO1 FPGA global interrupt enable reg*/
#define GL_IAR_OFFSET 0x0C /* CLS MRVL_CPO1 FPGA global interrupt ack reg*/
#define GL_MER_OFFSET \
    0x1C /* CLS MRVL_CPO1 FPGA global interrupt master enable reg*/
...
static void xiic_smbus_block_read_setup(struct xiic_i2c *i2c) {
    ...
    if (rxmsg_len <= I2C_SMBUS_BLOCK_MAX) {
        ...
        } else if ((rxmsg_len == 1) || (rxmsg_len == 0)) {
            /*
...
static void xiic_read_rx(struct xiic_i2c *i2c) {
    ...
    /* if bytes in fifo larger than bytes need to read, means 2 messages are
       mixed in fifo set bytes_to_read to the current message length
    */
    if (bytes_in_fifo > xiic_rx_space(i2c)) bytes_in_fifo = xiic_rx_space(i2c);
    ...
    if (!i2c->dynamic) {
        ...
        if (bytes_rem > IIC_RX_FIFO_DEPTH) {
            ...
            cr = xiic_getreg8(i2c, XIIC_CR_REG_OFFSET);
            xiic_setreg8(i2c, XIIC_CR_REG_OFFSET, cr | XIIC_CR_NO_ACK_MASK);
        } else if (bytes_rem == 0) {
            bytes_to_read = bytes_in_fifo;
            ...
            if (i2c->nmsgs == 1) {
                cr = xiic_getreg8(i2c, XIIC_CR_REG_OFFSET);
                xiic_setreg8(i2c, XIIC_CR_REG_OFFSET, cr & ~XIIC_CR_MSMS_MASK);
            }
...
static void xiic_fill_tx_fifo(struct xiic_i2c *i2c) {
    ...
    while (len--) {
        ...
        /* not send tx messages completely and the last byte to send, generate
         * stop bit after the last byte*/
...
static void xiic_send_tx(struct xiic_i2c *i2c) {
    dev_dbg(i2c->adap.dev.parent, "%s entry, rem: %d, SR: 0x%x, CR: 0x%x\n",
            __func__, xiic_tx_space(i2c), xiic_getreg8(i2c, XIIC_SR_REG_OFFSET),
            xiic_getreg8(i2c, XIIC_CR_REG_OFFSET));
...
static irqreturn_t xiic_process(int irq, void *dev_id) {
    ...
    dev_dbg(i2c->adap.dev.parent, "%s: SR: 0x%x, msg: %p, nmsgs: %d\n",
            __func__, xiic_getreg8(i2c, XIIC_SR_REG_OFFSET), i2c->tx_msg,
            i2c->nmsgs);
    ...
    if ((pend & XIIC_INTR_ARB_LOST_MASK) ||
         ...
         */
        ret = xiic_reinit(i2c);
        if (!ret) dev_dbg(i2c->adap.dev.parent, "reinit failed\n");

        if (i2c->rx_msg) xiic_wakeup(i2c, STATE_ERROR);
        if (i2c->tx_msg) xiic_wakeup(i2c, STATE_ERROR);
    }
    if (pend & XIIC_INTR_RX_FULL_MASK) {
        ...
        if (!i2c->rx_msg) {
            dev_dbg(i2c->adap.dev.parent, "%s unexpected RX IRQ\n", __func__);
    ...
    if (pend & (XIIC_INTR_TX_EMPTY_MASK | XIIC_INTR_TX_HALF_MASK)) {
        ...
        clr |= (pend & (XIIC_INTR_TX_EMPTY_MASK | XIIC_INTR_TX_HALF_MASK));

        if (!i2c->tx_msg) {
            dev_dbg(i2c->adap.dev.parent, "%s unexpected TX IRQ\n", __func__);
            goto out;
        }
    ...
    if (pend & XIIC_INTR_BNB_MASK) {
        ...
        if (!i2c->tx_msg) goto out;

        if (i2c->nmsgs == 1 && !i2c->rx_msg && xiic_tx_space(i2c) == 0)
...
static void xiic_start_recv(struct xiic_i2c *i2c) {
    ...
    if (i2c->dynamic) {
        ...
        /* Clear and enable Rx full interrupt. This interrupt is not pre-set in
         * reinit function */
        ...
        if (cr & XIIC_CR_MSMS_MASK) {
            ...
            xiic_setreg8(
                i2c, XIIC_CR_REG_OFFSET,
                (cr | XIIC_CR_REPEATED_START_MASK) & ~(XIIC_CR_DIR_IS_TX_MASK));
        }
    ...
    if (i2c->nmsgs == 1) /* very last, enable bus not busy as well */
        xiic_irq_clr_en(i2c, XIIC_INTR_BNB_MASK);
...
static void xiic_start_send(struct xiic_i2c *i2c) {
    ...
    dev_dbg(i2c->adap.dev.parent, "%s entry, msg: %p, len: %d", __func__, msg,
            msg->len);
    dev_dbg(i2c->adap.dev.parent, "%s entry, ISR: 0x%x, CR: 0x%x\n", __func__,
            xiic_getreg32(i2c, XIIC_IISR_OFFSET),
            xiic_getreg8(i2c, XIIC_CR_REG_OFFSET));
    ...
    if (i2c->dynamic) {
        ...
        if (!(msg->flags & I2C_M_NOSTART)) {
            /* write the address */
            data = i2c_8bit_addr_from_msg(msg) | XIIC_TX_DYN_START_MASK;
        ...
        /* Send one message, one message means these message with only one
...
static void __xiic_start_xfer(struct xiic_i2c *i2c) {
    ...
    while ((fifo_space >= 2) && (first || (i2c->nmsgs > 1))) {
        if (!first) {
            /* if it's not the first time enter loop,set offset and pass
             * i2c->tx_msg to xiic_start_send function again*/
            i2c->nmsgs--;
            i2c->tx_msg++;
            i2c->tx_pos = 0;
        } else {
...
static int xiic_start_xfer(struct xiic_i2c *i2c) {
    ...
    mutex_lock(&i2c->lock);
    /* IIC reset and all interrupts will be disable but ABR interrupt enable
     * here */
    ret = xiic_reinit(i2c);
    if (!ret) __xiic_start_xfer(i2c);
 ...
 */
static int ocores_wait(struct xiic_i2c *i2c, int reg, u8 mask, u8 val,
    ...
    while (1) {
        ...
        if (time_after(jiffies, j)) return -ETIMEDOUT;
        cpu_relax();
        cond_resched();
    }
    return 0;
...
static int ocores_poll_wait(struct xiic_i2c *i2c) {
    ...
    if (1 == i2c->nmsgs && 0 == i2c->msg->len && i2c->state == STATE_START &&
        ...
        mutex_unlock(&i2c->lock);
        if (0 != err) { /* AXI IIC as an transceiver , if ever an
                           XIIC_INTR_TX_ERROR_MASK interrupt happens, means no
                           such i2c device */
            err = 0;
        } else {
            err = -ETIMEDOUT;
        ...
        if (mask & XIIC_SR_TX_FIFO_EMPTY_MASK) {
            err = ocores_wait(i2c, XIIC_SR_REG_OFFSET, mask,
                              XIIC_SR_TX_FIFO_EMPTY_MASK, msecs_to_jiffies(30));
        ...
        if ((status & XIIC_INTR_ARB_LOST_MASK) ||
            ((status & XIIC_INTR_TX_ERROR_MASK) &&
             !(i2c->msg->flags &
               I2C_M_RD))) { /* AXI IIC as an transceiver , if ever an
                                XIIC_INTR_TX_ERROR_MASK interrupt happens,
                                return */
            err = -ETIMEDOUT;
            /* Soft reset IIC controller. */
            xiic_setreg32(i2c, XIIC_RESETR_OFFSET, XIIC_RESET_MASK);
...
static int ocores_xfer_core(struct xiic_i2c *i2c, struct i2c_msg *msgs,
                            int num) {
    int ret = 0, res = 0;
    u8 ctrl;
    ...
    if (msgs->len == 0 && num == 1) { /* suit for i2cdetect time sequence */
        ...
        xiic_setreg16(i2c, XIIC_DTR_REG_OFFSET,
                      i2c_8bit_addr_from_msg(msgs) | XIIC_TX_DYN_START_MASK |
...
static int xiic_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num) {
    ...
    if (i2c->flag == POLLING) {
        ...
        if (wait_event_timeout(
                i2c->wait,
                i2c->state == STATE_ERROR || i2c->state == STATE_DONE, HZ)) {
...
static int xiic_i2c_probe(struct platform_device *pdev) {
    struct xiic_i2c *i2c;
    /* not use pdata struct in linux/i2c-xiic.h but cls-i2c-xiic to pass
     * additional irq*/
    ...
    /* set pdev id and transfer it to interrupt base using specific formula in
     * isr function */
    i2c->id = pdev->id;
    ...
    /* 0 and 1 is for i801 and ismt driver, 2 is reserved for SMLink to act as
     * master if there's no BMC, so offset is 3 here*/
    ...
    if (!(sr & XIIC_SR_TX_FIFO_EMPTY_MASK)) i2c->endianness = BIG;
...
static const struct dev_pm_ops xiic_dev_pm_ops = {SET_RUNTIME_PM_OPS(
    xiic_i2c_runtime_suspend, xiic_i2c_runtime_resume, NULL)};
