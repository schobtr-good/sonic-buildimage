...
u32 wait_spi_busy (void __iomem *base_addr, u32 reg, u32 bitmask, u32 *done, u32 timeout_us, u32 sleeptime_us)
{
    u32 i;
    u32 timeout_count = timeout_us / sleeptime_us;
...
uint32_t obo_spi_read_mock (void __iomem *base_addr, u32 pim, u32 rtc_idx, uint32_t page, uint32_t start, uint8_t byte_len, uint8_t *buf)
{
    *buf = start;
...
uint32_t mrvl_obo_spi_read (void __iomem *base_addr, u32 obo_idx, u32 bank, uint32_t page, uint32_t start, uint8_t byte_len, uint8_t *buf)
{
    u32 i, done, ret;
    u32 spi_ctrl_reg, spi_stat_reg, spi_xfer_info_reg, spi_flow_ctrl_reg, data_w_reg, data_r_reg;
    ...
    spi_xfer_info_reg = MRVL_SPI_XFER_INFO_REG + (obo_idx * MRVL_SPI_CFG_REG_SIZE);
    spi_flow_ctrl_reg = MRVL_SPI_FLOW_CTRL_REG + (obo_idx * MRVL_SPI_CFG_REG_SIZE);
    ...
    PRINTK(KERN_INFO, "%s line#%d obo_idx: %d bank: %d page: %d offset: %d len: %d\n         \
                       ...
                       (long unsigned int)base_addr, (long unsigned int)spi_ctrl_reg, (long unsigned int)spi_stat_reg,
                       (long unsigned int)spi_xfer_info_reg, (long unsigned int)spi_flow_ctrl_reg,
                       (long unsigned int)data_w_reg, (long unsigned int)data_r_reg);
...
uint32_t mrvl_obo_spi_write (void __iomem *base_addr, u32 obo_idx, u32 bank, uint32_t page, uint32_t start, uint8_t byte_len, uint8_t *buf)
{
    u32 i, j, done, ret;
    u32 spi_ctrl_reg, spi_stat_reg, spi_xfer_info_reg, spi_flow_ctrl_reg, data_w_reg, data_r_reg;
    u32 spi_xfer_info_reg_val;
...
uint32_t obo_spi_read (void __iomem *base_addr, u32 pim, u32 rtc_idx, uint32_t page, uint32_t start, uint8_t byte_len, uint8_t *buf)
{
    ...
    PRINTK(KERN_INFO, "%s line#%d pim: %d rtc: %d page: %d offset: %d len: %d base_addr: 0x%lx pim_base: 0x%lx desc_l_reg: 0x%lx desc_h_reg: 0x%lx data_w_reg: 0x%lx data_r_reg: 0x%lx\n",
                     ...
                     (long unsigned int)base_addr, (long unsigned int)pim_base, (long unsigned int)desc_l_reg,
                     (long unsigned int)desc_h_reg, (long unsigned int)data_w_reg, (long unsigned int)data_r_reg);
...
uint32_t mrvl_spi_check_status(void __iomem *base_addr, uint32_t obo_idx, u32 bank)
{
    ...
    for (i = 0; i < 5; i++) {
        ret = mrvl_obo_spi_read(base_addr, obo_idx, bank, page, offset, byte_len, buf);
        if (ret == 0) {
            ret = 0;
            break;
        } else {
