...
static inline void port_setreg(struct xcvr_priv *xcvr, int reg, int index,
                               u8 value) {
    return iowrite8(value,
                    xcvr->base + reg + (index - 1) * xcvr->port_reg_size);
}
...
static struct device *init_port(struct device *dev, struct xcvr_priv *xcvr,
                                struct port_info info,
                                const struct attribute_group **groups) {
    struct port_data *new_data;

    new_data = devm_kzalloc(dev, sizeof(struct port_data), GFP_KERNEL);
    if (!new_data) return ERR_PTR(-ENOMEM);

    new_data->index = info.index;
    new_data->name = info.name;
    new_data->xcvr = xcvr;

    return devm_hwmon_device_register_with_groups(dev, info.name, new_data,
                                                  groups);
}
...
static int cls_xcvr_probe(struct platform_device *pdev) {
    ...
    port_devs = devm_kzalloc(
        &pdev->dev, xcvr->num_ports * sizeof(struct device *), GFP_KERNEL);
    if (!port_devs) {
        ret = -ENOMEM;
        goto err_exit;
    }
    ...
    if (pdata) {
        ...
        for (i = 0; i < pdata->num_ports; i++) {
            ...
            if (pdata->devices[i].type == SFP) {
                fp_dev =
                    init_port(&pdev->dev, xcvr, pdata->devices[i], sfp_groups);
            } else {
                fp_dev =
                    init_port(&pdev->dev, xcvr, pdata->devices[i], qsfp_groups);
            }
            if (IS_ERR(fp_dev)) {
            ...
            dev_info(&pdev->dev, "Register port %s\n", pdata->devices[i].name);

            WARN(sysfs_create_link(&pdev->dev.kobj, &fp_dev->kobj,
                                   pdata->devices[i].name),
