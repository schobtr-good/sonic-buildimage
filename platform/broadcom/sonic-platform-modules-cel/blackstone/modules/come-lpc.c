/*
 * come-lpc.c - The CPLD driver for the COME of Blackstone
 * The driver implement sysfs to access CPLD register on the COME of Blackstone via LPC bus.
...
static void cpld_c_dev_release(struct device *dev)
{
    return;
}
...
static int cpld_c_drv_probe(struct platform_device *pdev)
{
    ...
    cpld_data = devm_kzalloc(&pdev->dev, sizeof(struct cpld_c_data),
                             GFP_KERNEL);
    if (!cpld_data)
        return -ENOMEM;
...
MODULE_DESCRIPTION("Celestica Blackstone CPLD COME driver");
