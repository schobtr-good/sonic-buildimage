/*
 * baseboard-lpc.c - The CPLD driver for the Base Board of Blackstone
 * The driver implement sysfs to access CPLD register on the baseboard of Blackstone via LPC bus.
...
#define REGISTER_SIZE       0xA3

#define LED_OFF_STR         "off"
#define LED_ON_STR          "on"
#define LED_4HZ_STR         "4hz"
#define LED_1HZ_STR         "1hz"
#define LED_YELLOW_STR      "yellow"
#define LED_GREEN_STR       "green"
#define LED_BOTH_STR        "both"
                ...
                char *buf)
{
    ...
    return sprintf(buf, "%s\n",
            data == 0x03 ? LED_OFF_STR : data == 0x02 ? LED_4HZ_STR : data ==0x01 ? LED_1HZ_STR : LED_ON_STR);
}
                ...
                const char *buf, size_t count)
{
    ...
    if(sysfs_streq(buf, LED_OFF_STR)){
        led_status = 0x03;
    }else if(sysfs_streq(buf, LED_4HZ_STR)){
        led_status = 0x02;
    }else if(sysfs_streq(buf, LED_1HZ_STR)){
        led_status = 0x01;
    }else if(sysfs_streq(buf, LED_ON_STR)){
        led_status = 0x00;
    }else{
        count = -EINVAL;
        return count;
                ...
                char *buf)
{
    ...
    data = (data >> 4) & 0x3;
    return sprintf(buf, "%s\n",
            data == 0x03 ? LED_OFF_STR : data == 0x02 ? LED_YELLOW_STR : data ==0x01 ? LED_GREEN_STR : LED_BOTH_STR);
}
                ...
                const char *buf, size_t count)
{
    ...
    if(sysfs_streq(buf, LED_OFF_STR)){
        led_status = 0x03;
    }else if(sysfs_streq(buf, LED_YELLOW_STR)){
        led_status = 0x02;
    }else if(sysfs_streq(buf, LED_GREEN_STR)){
        led_status = 0x01;
    }else if(sysfs_streq(buf, LED_BOTH_STR)){
        led_status = 0x00;
    }else{
        count = -EINVAL;
        return count;
...
MODULE_DESCRIPTION("Celestica Blackstone CPLD baseboard driver");
