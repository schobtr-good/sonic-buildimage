...
struct pddf_fan_data {
    struct device   *hwmon_dev;
    struct mutex     update_lock;
    char             valid;           /* != 0 if registers are valid */
    unsigned long    last_updated;    /* In jiffies */
    u8               reg_val[ARRAY_SIZE(pddf_fan_reg)]; /* Register value */
};
...
static struct attribute *pddf_fan_attributes[] = {
            ...
            const char *buf, size_t count)
{
    ...
    fan_write_value(client, pddf_fan_reg[attr->index], value);
    return count;
}
...
static const struct attribute_group pddf_fan_group = {
    .attrs = pddf_fan_attributes,
};

static struct pddf_fan_data *fan_update_device(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct pddf_fan_data *data = i2c_get_clientdata(client);
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) || 
        ...
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = fan_read_value(client, pddf_fan_reg[i]);
            
            if (status < 0) {
                data->valid = 0;
                mutex_unlock(&data->update_lock);
                dev_dbg(&client->dev, "reg %d, err %d\n", pddf_fan_reg[i], status);
                return data;
            }
            else {
            ...
            const struct i2c_device_id *dev_id)
{
    ...
    data = kzalloc(sizeof(struct pddf_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &pddf_fan_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &pddf_fan_group);
exit_free:
    kfree(data);
exit:
