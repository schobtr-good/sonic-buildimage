...
#define PCA9534_INPUT_REG   (0)
#define PCA9534_OUTPUT_REG   (1)
#define PCA9534_POL_INV_REG   (2)
#define PCA9534_CFG_REG   (3)
#define PCA9534_I2C_BUS   (0)
...
char IOx_map[] = {
1,//IOx index of pca9534 corresponding to CPLD_BB
2,//IOx index of pca9534 corresponding to CPLD_CB
0,//IOx index of pca9534 corresponding to CPLD_FCB
3,//IOx index of pca9534 corresponding to CPLD_SWB
4,//IOx index of pca9534 corresponding to CPLD_LCT
5,//IOx index of pca9534 corresponding to CPLD_LCB
};
/***************************************************************
*
...
/*Description: set pca9534 IO0~IO7 output level
*param_in: io_index - index of IO pin
*param_out:NONE
*ret_val: 0 - on successful
*         -1 - on failed
*/
int set_pca9534_output(int io_index)
{
    int sys_cmd_rc = 0;
    char sys_cmd[SYS_CMD_LEN] = {0};
    char reg_val = 0xFF & (~(1 << IOx_map[io_index]));
    printf("set pca9534 IO%d to level 0 \n", IOx_map[io_index]);
    memset(sys_cmd, 0, SYS_CMD_LEN);
    sprintf(sys_cmd, "i2cset -y %d 0x%x 0x%x 0x%x", PCA9534_I2C_BUS,
                      PCA9534_I2C_ADDR, PCA9534_OUTPUT_REG, 0xff & reg_val);
    sys_cmd_rc = exec_cmd(sys_cmd);
    if (sys_cmd_rc != 0) {
       printf("set pca9534 IO%d to level 0 failed!\n", IOx_map[io_index]);
       return -1;
    }
    return sys_cmd_rc;
}
...
int main( int argc, char * argv[] )
{
    ...
    /* ChannelIndex: indicate which channel on PCA9534 is selected */
    int ChannelIndex                = -1;
    char sys_cmd[SYS_CMD_LEN] = {0};
    int setChannel = 0;
    int sys_cmd_rc = 0;
    ...
    if (ChannelIndex != -1) {
       if (ChannelIndex < 0 || ChannelIndex > 7) {
           printf("Invalid pca9534 channel index! \n");
           print_usage(argv[0]);
           return -1;
       } else {
           printf("pca9534 channel index: %d \n", ChannelIndex);  
           setChannel = 1;
         }
    } else {
           printf("pca9534 output channel not set,use default value 0 ! \n");
           ChannelIndex = 0;
           setChannel = 1;
      }
    ...
    */
    ...
    if (setChannel) {
       printf("Set pca9534 output channel index to %d \n", ChannelIndex);
       memset(sys_cmd, 0, SYS_CMD_LEN);
       sprintf(sys_cmd, "i2cset -y %d 0x%x 0x%x 0x00", PCA9534_I2C_BUS, PCA9534_I2C_ADDR, PCA9534_CFG_REG);
       sys_cmd_rc = exec_cmd(sys_cmd);
       if (sys_cmd_rc != 0) {
          printf("set pca9534 IO0~IO7 to output mode failed!\n");
          return -1;
       }
       sys_cmd_rc = set_pca9534_output(ChannelIndex);
       if (sys_cmd_rc != 0) {
           printf("set pca9534 IO%d output 0 failed!\n", IOx_map[ChannelIndex]);
           return -1;
       }
    }
